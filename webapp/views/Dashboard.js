// In-flight promise caches for deduping user and latest snapshot fetches
const _inflightFetchMe = { promise: null, result: null };
const _inflightFetchLatest = { promise: null, result: null };

// Helper to fetch /admin/api/me with deduplication
async function fetchMeDedupe(token, doApiFetch) {
  if (_inflightFetchMe.result) return _inflightFetchMe.result;
  if (_inflightFetchMe.promise) return _inflightFetchMe.promise;
  _inflightFetchMe.promise = (async () => {
    let res = null;
    try {
      res = await doApiFetch('/admin/api/me', { headers: token ? { Authorization: `Bearer ${token}`, Accept: 'application/json' } : undefined });
      if (res && res.ok) {
        const js = await res.json();
        _inflightFetchMe.result = js;
        return js;
      }
    } catch (e) {}
    _inflightFetchMe.result = null;
    return null;
  })();
  try {
    return await _inflightFetchMe.promise;
  } finally {
    _inflightFetchMe.promise = null;
  }
}

// Helper to fetch /api/latest with deduplication (if used)
async function fetchLatestDedupe(token, doApiFetch) {
  if (_inflightFetchLatest.result) return _inflightFetchLatest.result;
  if (_inflightFetchLatest.promise) return _inflightFetchLatest.promise;
  _inflightFetchLatest.promise = (async () => {
    let res = null;
    try {
      res = await doApiFetch('/api/latest', { headers: token ? { Authorization: `Bearer ${token}`, Accept: 'application/json' } : undefined });
      if (res && res.ok) {
        const js = await res.json();
        _inflightFetchLatest.result = js;
        return js;
      }
    } catch (e) {}
    _inflightFetchLatest.result = null;
    return null;
  })();
  try {
    return await _inflightFetchLatest.promise;
  } finally {
    _inflightFetchLatest.promise = null;
  }
}
import React, { useEffect, useState, useRef, useMemo } from 'react';
import { apiFetch } from '../src/api';
import { API_HOST, MQTT_WS_HOST, MQTT_WS_PATH, MQTT_WS_LEGACY_PATH, apiUrl, wsUrl } from '../src/hosts';
...
// Toggle to control whether the dashboard will automatically persist discovered
// canonical bases and placeholder power labels back to the server. Default is
// true to enable production-like behavior where discovered bases become
// authoritative in the topics DB. Developers may override by setting
// `window.__BREWSKI_ENABLE_AUTO_PERSIST__ = false` in the console for testing.
const ENABLE_AUTO_PERSIST = (typeof window !== 'undefined' && typeof window.__BREWSKI_ENABLE_AUTO_PERSIST__ !== 'undefined') ? !!window.__BREWSKI_ENABLE_AUTO_PERSIST__ : true;
...
const DEFAULT_API_HOST = API_HOST || 'api.brewingremote.com';
...
async function fetchPowerLabels(token) {
        ...
        try {
          const r = await window.apiFetch('/admin/api/power-labels');
              if (r && r.ok) {
                ...
                // Return the full server response so callers can inspect labels_by_canonical
                return js || null;
          }
  ...
  const adminUrl = apiUrl('/admin/api/power-labels');
  const publicUrl = apiUrl('/api/power-labels');
...
function canonicalCandidatesForTopic(topic) {
  ...
  const candidates = [];
  try {
    // Attempt to compute canonical stored form using canonicalForTopic helper
    try {
      const stored = canonicalForTopic(topic);
      if (stored) candidates.push(stored);
    } catch (e) {}
    // Always include exact provided topic as a fallback
...
function normalizeCanonicalBase(origKey, { knownSlugs, gMeta, dbSensorBases, customerSlug, mode } = {}) {
  try {
    ...
    if (partsRaw.length >= 2 && partsRaw[0] && partsRaw[1]) {
      ...
      // If site is non-BREW, accept it. If site is BREW, do not attempt to
      // substitute a non-BREW variant from db/gMeta; preserve the explicit
      // BREW site as provided and let server-side canonicalization be authoritative.
  ...
  const telMatch = String(origKey).match(/^tele\/(?:([^/]+)\/)?([^/]+)(?:\/.*)?$/i) || String(origKey).match(/^stat\/(?:([^/]+)\/)?([^/]+)(?:\/.*)?$/i);
    if (telMatch) {
      ...
      // Prefer explicit mode/customerSlug if available. Do not attempt to
      // infer a non-BREW site from DB or gMeta here; that introduces implicit
      // legacy fallbacks.
      if (!device || /^(STATE|BREW|RAIL)$/i.test(String(device))) return null;
  // When no site is known, prefer mode/customerSlug if provided. If neither
  // is available, return null to indicate a canonical base cannot be determined.
...
export default function Dashboard({ token, onCustomerLoaded }) {
  ...
  // DEBUG: Log key state for diagnosing empty dashboard (after all state/memo declarations)
  if (typeof window !== 'undefined') {
    try { console.log('[Dashboard DEBUG] dbSensorBases:', Array.from(dbSensorBases)); } catch (e) {}
    try { console.log('[Dashboard DEBUG] deviceList:', deviceList); } catch (e) {}
    try { console.log('[Dashboard DEBUG] filteredDevices:', filteredDevices); } catch (e) {}
    try { console.log('[Dashboard DEBUG] customerSlug:', customerSlug, 'mode:', mode); } catch (e) {}
  }
  ...
  useEffect(() => {
    let mounted = true;
    (async () => {
      const labelsResp = await fetchPowerLabels(token);
      // labelsResp may be null, an object { labels: [...], labels_by_canonical: {...} },
      // or older array-shaped responses. Normalize to an array + canonical map.
      const labelsArr = Array.isArray(labelsResp) ? labelsResp : (labelsResp && Array.isArray(labelsResp.labels) ? labelsResp.labels : null);
      const serverCanonical = labelsResp && labelsResp.labels_by_canonical ? labelsResp.labels_by_canonical : null;
  ...
  if (Array.isArray(labelsArr)) {
      ...
      if (mounted) {
        ...
        const canonicalMap = {};
        // If server provided labels_by_canonical prefer that authoritative map
        if (serverCanonical && typeof serverCanonical === 'object') {
          try {
            Object.keys(serverCanonical).forEach(k => { canonicalMap[k] = Object.assign({}, serverCanonical[k]); });
          } catch (e) {}
        }
        if (Array.isArray(labelsArr)) {
          labelsArr.forEach(l => {
            try {
              ...
              const can = canonicalForTopic(l.topic) || normalizeCanonicalBase((String(l.topic).split('/').filter(Boolean).slice(-2).join('/')) || l.topic, { knownSlugs, gMeta, dbSensorBases, customerSlug, mode }) || l.topic;
              if (!can) return;
  ...
  if (DEBUG) console.log('Dashboard: setting powerLabels from fetch', { count: Object.keys(labelMap || {}).length, sample: Object.keys(labelMap || {}).slice(0,5) });
        ...
        // Merge server canonical map into client state (prefer server values)
        setPowerLabelsByCanonical(prev => {
          try {
            const next = Object.assign({}, prev || {});
            Object.keys(canonicalMap || {}).forEach(k => { next[k] = Object.assign({}, next[k] || {}, canonicalMap[k] || {}); });
            return next;
  ...
  // Listen for global canonical map broadcasts (emitted by AdminPortal after saves)
  useEffect(() => {
    const handler = (ev) => {
      try {
        const map = ev && ev.detail && ev.detail.labels_by_canonical;
        if (!map) return;
        setPowerLabelsByCanonical(prev => {
          try {
            const next = Object.assign({}, prev || {});
            Object.keys(map).forEach(k => { next[k] = Object.assign({}, next[k] || {}, map[k] || {}); });
            return next;
          } catch (e) { return prev; }
        });
      } catch (e) {}
    ...
    try {
      if (typeof window !== 'undefined' && typeof window.addEventListener === 'function') window.addEventListener('brewski:power-labels-canonical', handler);
    } catch (e) {}
    return () => { try { if (typeof window !== 'undefined' && typeof window.removeEventListener === 'function') window.removeEventListener('brewski:power-labels-canonical', handler); } catch (e) {} };
  }, []);
...
function parseJwtPayload(tok) {
  try {
    ...
    try {
      // Derive known slugs from JWT payload if possible. Avoid calling /api/companies
      // which may be legacy or removed.
      if (token) {
        const p = parseJwtPayload(token);
        const maybe = (p && ((p.customer && p.customer.slug) || p.customer_slug || p.site || p.org)) ? (p.customer && p.customer.slug ? p.customer.slug : (p.customer_slug || p.site || p.org)) : null;
        if (maybe && typeof maybe === 'string') return new Set([String(maybe).toUpperCase()]);
      }
      return new Set();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const js = await fetchMeDedupe(token, doApiFetch);
        if (mounted && js && js.customer && js.customer.slug) {
  ...
  const findSiteForDevice = (deviceName) => {
    ...
    // Incoming update batching to reduce re-renders (especially on native)
    const pendingSensorsRef = useRef({}); // canonical -> value
    const pendingTargetsRef = useRef({});
    const pendingPowerRef = useRef({}); // canonical -> { POWER: bool, POWER1: bool }
    const pendingMetaRef = useRef({});
    const pendingFlushTimerRef = useRef(null);
    const FLUSH_MS = 120; // ms

    const schedulePendingFlush = () => {
      if (pendingFlushTimerRef.current) return;
      pendingFlushTimerRef.current = setTimeout(() => {
        try {
          const s = pendingSensorsRef.current;
          const t = pendingTargetsRef.current;
          const p = pendingPowerRef.current;
          const m = pendingMetaRef.current;
          // apply sensors
          if (Object.keys(s).length) {
            setGSensors(prev => ({ ...prev, ...s }));
          }
          // apply targets
          if (Object.keys(t).length) {
            setGTargets(prev => ({ ...prev, ...t }));
          }
          // apply meta
          if (Object.keys(m).length) {
            setGMeta(prev => ({ ...prev, ...m }));
          }
          // merge power states per-base
          if (Object.keys(p).length) {
            setGPower(prev => {
              const next = { ...prev };
              for (const k of Object.keys(p)) {
                next[k] = Object.assign({}, next[k] || {}, p[k]);
              }
              return next;
            });
          }
        ...
        pendingSensorsRef.current = {};
        pendingTargetsRef.current = {};
        pendingPowerRef.current = {};
    ...
    // Helper adders used by message handlers to batch updates
    const queueSensor = (canonical, val) => {
      if (!canonical) return;
      pendingSensorsRef.current[canonical] = val;
      schedulePendingFlush();
    };
    const queueTarget = (canonical, val) => {
      if (!canonical) return;
      pendingTargetsRef.current[canonical] = val;
      schedulePendingFlush();
    };
    const queueMeta = (canonical, meta) => {
      if (!canonical || !meta) return;
      pendingMetaRef.current[canonical] = Object.assign({}, pendingMetaRef.current[canonical] || {}, meta);
      schedulePendingFlush();
    };
    const queuePower = (canonical, foundMap) => {
      if (!canonical || !foundMap || !Object.keys(foundMap).length) return;
      const cur = pendingPowerRef.current[canonical] || {};
      Object.assign(cur, foundMap);
      pendingPowerRef.current[canonical] = cur;
      schedulePendingFlush();
    };

    // Fallback: allow device-only /Sensor topics to be registered as <mode>/<device>/Sensor if mode/customerSlug is known
    // This enables reliable rendering for both web and native clients
    const registerDeviceOnlySensor = (deviceName, value) => {
      if (!deviceName) return;
      const site = mode || (customerSlug ? String(customerSlug).toUpperCase() : null);
      if (!site) return;
      const canonical = `${site}/${deviceName}`;
      setGSensors(prev => ({ ...prev, [canonical]: value }));
      setGMeta(prev => ({ ...prev, [canonical]: { site, device: deviceName, metric: null, terminal: 'Sensor' } }));
      // Locally add to dbSensorBases so UI can render immediately
      setDbSensorBases(prev => {
        const s = new Set(Array.from(prev || []));
        s.add(canonical);
        return s;
      });
      // Persist base using slug if numeric customerId is missing
      persistBaseToDB(canonical, `${deviceName}/Sensor`, { site, device: deviceName, metric: null, terminal: 'Sensor' });
    };
  ...
  const persistBaseToDB = async (baseKey, rawTopic, meta, { force = false } = {}) => {
      try {
        // Short-circuit when auto-persist is disabled unless explicitly forced
        // by a user action (force=true). This allows manual persistence from UI.
        if (!ENABLE_AUTO_PERSIST && !force) {
          if (DEBUG) console.debug('Dashboard: persistBaseToDB skipped (auto-persist disabled)', baseKey);
          return false;
        }
        ...
        let path, payload;
        if (customerId) {
          path = `/admin/api/customers/${encodeURIComponent(customerId)}/topics`;
          payload = {
          ...
          path = `/admin/api/customers/topics`;
          payload = {
            topic_key: rawTopic || baseKey,
            site: meta && meta.site ? meta.site : (baseKey && baseKey.split('/')[0]) || null,
            device: meta && meta.device ? meta.device : (baseKey && baseKey.split('/')[1]) || null,
          ...
          if (DEBUG) console.warn('Dashboard: cannot persist base without customerId or customerSlug', { baseKey });
          return false;
        }
        // Use the component-level doApiFetch helper so all admin/public API calls are routed consistently to the canonical API host.
  ...
  const persistDiscoveredPowerKeys = async (baseKey, powerStates, { force = false } = {}) => {
      try {
        // Respect auto-persist gate unless forced by a user action
        if (!ENABLE_AUTO_PERSIST && !force) {
          if (DEBUG) console.debug('Dashboard: persistDiscoveredPowerKeys skipped (auto-persist disabled)', baseKey);
          return false;
        }
      ...
      // Explicit user-triggered persistence: persist all discovered canonical bases
      // and power keys. This is intended to be invoked from an admin UI control
      // so discovery remains read-only by default.
      const persistAllDiscovered = async () => {
        if (!token) return false;
        try {
          // Build a set of canonical bases from current gPower keys
          const bases = new Set();
          ...
          // Also include deviceList canonical keys
          (deviceList || []).forEach(d => { try { if (d && d.key) bases.add(d.key); } catch (e) {} });

          for (const b of Array.from(bases)) {
            try {
              // Attempt to persist base (force)
              await persistBaseToDB(b, b, null, { force: true });
              // Persist any discovered power keys for this base
              const states = gPower[b] || gPower[b + '/STATE'] || gPower[b.replace(/^BREW\//i, '')] || null;
              if (states) await persistDiscoveredPowerKeys(b, states, { force: true });
            } catch (e) { /* per-base non-fatal */ }
          }
          // Refresh labels cache after posts
          try { const labelsArr = await fetchPowerLabels(token); if (labelsArr) { /* no-op: fetchPowerLabels caches */ } } catch (e) {}
          return true;
        } catch (e) { return false; }
      };
  ...
  const FORCED_HOST = '172.20.10.5'; // <-- hardcoded LAN IP (left for dev overrides)
  // Use centralized MQTT WS host/path defaults from hosts.js
  const USE_PUBLIC_WS = true; // flip to `false` for local LAN development
  const PUBLIC_WS_HOST = MQTT_WS_HOST || API_HOST || 'api.brewingremote.com';
  const PUBLIC_WS_PATH = MQTT_WS_PATH || '/_ws';
          ...
          for (const [baseKeyRaw, states] of entries) {
            try {
              ...
              if (syncedPowerRef.current.has(cacheKey)) continue;
                  // Persist canonical base and discovered power keys only when auto-persist is enabled.
                  if (ENABLE_AUTO_PERSIST) {
                    ...
                    if (DEBUG) console.debug('Dashboard: auto-persist disabled; skipping persist for', baseKey);
                  }
  ...
  const getPowerLabel = (baseKey, powerKey) => {
    ...
    try {
      ...
      // 2) Direct exact matches only (exact topic|POWERKey)
      const direct = `${baseKey}|${pk}`;
      if (powerLabels[direct] !== undefined && String(powerLabels[direct] || '').trim().length) return powerLabels[direct];
      const directU = `${baseKey.toUpperCase()}|${pk}`;
      if (powerLabels[directU] !== undefined && String(powerLabels[directU] || '').trim().length) return powerLabels[directU];

      // 3) Check canonical SITE/DEVICE keys in powerLabels (canonical or canonical/STATE)
      try {
        const can = canonicalForTopic(baseKey) || null;
        if (can) {
          const canonicalKey = `${can}|${pk}`;
          const canonicalStateKey = `${can}/STATE|${pk}`;
          if (powerLabels[canonicalKey] !== undefined && String(powerLabels[canonicalKey] || '').trim().length) return powerLabels[canonicalKey];
          const cU = `${canonicalKey.toUpperCase()}`;
          if (powerLabels[cU] !== undefined && String(powerLabels[cU] || '').trim().length) return powerLabels[cU];
          if (powerLabels[canonicalStateKey] !== undefined && String(powerLabels[canonicalStateKey] || '').trim().length) return powerLabels[canonicalStateKey];
          const csU = `${canonicalStateKey.toUpperCase()}`;
          if (powerLabels[csU] !== undefined && String(powerLabels[csU] || '').trim().length) return powerLabels[csU];
        }
      } catch (e) {}
    } catch (e) {}
    ...
    try {
      ...
      if (hasAny) {
        if (DEBUG) console.warn('Dashboard: getPowerLabel - no label found', { baseKey, powerKey });
      } else {
  ...
  const resolveHost = () => {
    // prefer explicit hosts from hosts.js; fall back to FORCED_HOST for LAN dev
    if (MQTT_WS_HOST) return MQTT_WS_HOST;
    ...
    try { if (DEFAULT_API_HOST) return DEFAULT_API_HOST; } catch (e) {}
    return 'api.brewingremote.com';
  };
  ...
  const connectWebSocket = async () => {
    ...
    // Prefer secure wss for non-local hosts. For local development (localhost/127.x)
    // use plain ws so the client doesn't attempt TLS against a non-TLS dev server.
    const isLocalHost = (() => {
      try {
        if (!host) return false;
        const h = String(host).toLowerCase();
        if (h.startsWith('localhost') || h.startsWith('127.') || h.startsWith('0.0.0.0')) return true;
        // allow explicit IPv4 localhost with port (127.0.0.1:8080)
        if (h.indexOf('127.') === 0) return true;
        return false;
      } catch (e) { return false; }
    })();
    const proto = isLocalHost ? 'ws' : 'wss';
    const base = `${proto}://${host}${PUBLIC_WS_PATH}`;
  ...
  safeSend(ws, { type: 'inventory', id: 'initial-inventory' });
        ...
        // Query power states for known devices (derive device list dynamically)
            const queryAllPowerStates = () => {
          // derive device names from the current deviceList (canonical bases -> device name)
          const deviceNames = (deviceList || []).map(d => {
            try { const parts = String(d.key || '').split('/').filter(Boolean); return parts.length ? parts[parts.length-1] : d.key; } catch (e) { return d.key; }
          }).filter(Boolean);
          deviceNames.forEach(deviceName => {
            ...
            // Query additional power states for multi-switch devices (derive common candidates dynamically)
            const multiSwitchCandidates = (deviceNames || []).filter(n => /MASH|HLT|BOIL/i.test(String(n)));
            if (multiSwitchCandidates.includes(deviceName)) {
              for (let i = 1; i <= 3; i++) {
                try {
                  const site = (mode || (customerSlug ? String(customerSlug).toUpperCase() : null));
        ...
        const retryTimer = setInterval(() => {
          ...
          const bases = (deviceList || []).map(d => d.key);
          bases.forEach(base => {
  ...
  ws.onmessage = (ev) => {
        try {
          ...
          const canonicalBaseFromMeta = (m) => {
            ...
            if (!meta) return;
            // If site missing, prefer discovery inference; otherwise leave undefined
            if (!meta.site) {
              const inferred = findSiteForDevice(meta.device);
              meta.site = inferred || null;
            }
            ...
            const enrichedMeta = { site: (meta.site || (mode && mode !== 'BREW' ? mode : (customerSlug || null))), device: meta.device, metric: meta.metric || null, terminal: 'Sensor' };
            ...
            const queryPowerStates = (customerSlug, deviceName) => {
              ...
              // Probe Power1..Power3 for every device to discover multi-switch devices
              // rather than relying on a maintained list of known multi-switch names.
              for (let i = 1; i <= 3; i++) {
                try {
          ...
          const applyTarget = (topic, val) => {
            ...
            const enrichedMeta = { site: (meta.site || (mode && mode !== 'BREW' ? mode : (customerSlug || null))), device: meta.device, metric: meta.metric || null, terminal: 'Target' };
            registerMeta(canonical, enrichedMeta);
          };
          ...
          const applyPower = (topic, payloadObj) => {
            ...
            // Legacy Pattern: tele/<device>/(STATE|RESULT) - direct device (being phased out)
              ...
              deviceName = parts[1];
              if (deviceName) {
                // Try to infer site from existing meta first, else prefer explicit mode, otherwise leave site unknown
        const inferred = findSiteForDevice(deviceName);
        const siteForLegacy = inferred || mode || null;
        // Treat missing site as unknown; only build baseKey when site is known
        baseKey = siteForLegacy ? `${siteForLegacy}/${deviceName}` : null;
              }
            }
            // Response Pattern: stat/<device>/(STATE|RESULT) - device control responses
              ...
              if (deviceName) {
                const inferred = findSiteForDevice(deviceName);
                const siteForStat = inferred || mode || (customerSlug ? String(customerSlug).toUpperCase() : null);
                // Require a site to form a canonical base for stat responses
          ...
          const applyStatPower = (topic, rawPayload) => {
            try {
              ...
              // Require explicit site token; do not default to BREW or infer site
              if (!site) return;
              const normBase = normalizeCanonicalBase(`${site}/${device}`, { knownSlugs, gMeta, dbSensorBases, customerSlug, mode });
              if (!normBase) return;
          ...
          if (obj.type === 'current' && obj.topic === 'DUMMYtest/Target') {
            ...
            setGTargets(prev => {
              const next = { ...prev };
              (deviceList || []).forEach(d => { try { next[d.key] = n; } catch (e) {} });
              return next;
            });
          ...
          if (obj.type === 'grouped-inventory' && obj.data && typeof obj.data === 'object') {
            ...
            Object.values(groups).forEach(g => {
              ...
              Object.entries(g.topics).forEach(([topic, val]) => {
                  ...
                  if (!parsed) return;
                  // Require explicit site token in grouped inventory; skip entries lacking site
              ...
              if (typeof obj.topic === 'string' && obj.topic.toLowerCase().startsWith('tasmota/discovery/')) {
                ...
                if (tfield) {
                  try {
                    ...
                    if (device && site) {
                      const canonical = `${site}/${device}`;
                      setGMeta(prev => ({ ...prev, [canonical]: { site, device, metric: null } }));
        ...
        const parsed = parseTopic(topic);
        if (!parsed) return;
          // Require explicit site; skip snapshot entries lacking a site
          if (!parsed.site) return;
          const baseKey = canonicalBaseFromMeta(parsed) || (parsed.metric ? `${parsed.site}/${parsed.device}/${parsed.metric}` : `${parsed.site}/${parsed.device}`);
              const n = Number(val);
  ...
  useEffect(() => {
    ...
    // Thresholds endpoint removed/deprecated; keep thresholds empty unless other sources populate them
    (async () => { try { setThresholds({}); } catch (e) {} })();
  ...
  if (Array.isArray(statLists) && statLists.length) {
    ...
    if (DEBUG) console.debug('Dashboard: no server-side stat list found in /api/latest response; relying on sensor rows only');
        ...
        js.sensors.forEach(row => {
          ...
          // Require explicit site in snapshot rows; skip rows without site
          if (!site) return;
          ...
          if (parsedLastRaw && typeof parsedLastRaw === 'object') {
            try {
              ...
              if (Object.keys(found).length) {
                const baseCandidate = (site && device) ? `${site}/${device}` : rawTopic;
  ...
  useEffect(() => {
    ...
    // Query power states for all known devices. Derive device names from the
    // runtime-discovered `deviceList` instead of relying on a hard-coded array.
    // For multi-switch devices we probe Power1..Power3 so discovery detects
    // any additional named switches without embedding legacy device name lists.
    const queryPowerStatesForCustomer = () => {
      const deviceNames = (deviceList && deviceList.length) ? deviceList.map(d => {
        try {
          const k = String(d && d.key ? d.key : d).split('/').filter(Boolean);
          return k.length >= 2 ? k[1] : (k[0] || String(d && d.key ? d.key : d));
        } catch (e) {
          return String(d && d.key ? d.key : d);
        }
      }) : [];

      // If we don't have any discovered devices yet, do nothing (avoid
      // probing arbitrary defaults in modern runs). This keeps behaviour
      // predictable in new deployments and during dev where discovery may
      ...
      deviceNames.forEach(deviceName => {
        ...
        // Probe Power1..Power3 for every device to detect multi-switch devices
        // without needing a maintained list of known multi-switch names.
        for (let i = 1; i <= 3; i++) {
          try {
            const site = mode || (customerSlug ? String(customerSlug).toUpperCase() : null);
            const t = buildCmdTopic(site, deviceName, `Power${i}`);
  ...
  const publishTargetForDevice = (deviceKey, n) => {
    ...
    // Only publish to the new targ/<SITE>/<DEVICE>Target topic
    let pubTopic = null;
    try {
      const sitePart = site || (mode ? mode : (customerSlug ? String(customerSlug).toUpperCase() : ''));
      pubTopic = `targ/${sitePart}/${deviceName}Target`;
    } catch (e) {
  ...
  // Build a cmnd topic. Returns `cmnd/<site>/<device>/<Cmd>` when site is provided,
  // otherwise `cmnd/<device>/<Cmd>` as a last-resort legacy fallback.
  ...
  const requestCurrentForBase = (base) => {
    try {
      ...
      // If site missing or the site is BREW, optionally probe for additional legacy variants
      try {
  ...
  {/* In-app debug panel for RN / DEBUG mode (removed) */}
      {DEBUG && (
        ...
        {/* Admin control: persist discovered bases & power keys (visible in DEBUG or when token present) */}
        {/* Persist discovered button removed as requested */}
